...
const keywordMatches = (text: string): boolean => {
  const keywords = ["beurt", "bird", "burt", "bert", "plus", "score", "+", "streak", "reeks", "poedel"]
  return keywords.some((keyword) => text.toLowerCase().startsWith(keyword)) || text.toLowerCase().trim() === "poedel"
}
...
const App: React.FC = () => {
    ...
    if (savedTurns) {
      try {
        ...
        if (
          ...
          parsedTurns.every((turn) => typeof turn.id === "number" && typeof turn.points === "number")
        ) {
          return parsedTurns
        }
      } catch (error) {
  ...
  const editTurn = useCallback((id: number, newPoints: number) => {
    setTurns((prevTurns) => {
      const newTurns = prevTurns.map((turn) => (turn.id === id ? { ...turn, points: newPoints } : turn))
      return newTurns
    })
  }, [])
    ...
    <div className="max-w-4xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between mb-4">
          <button
            ...
            className={`mb-4 sm:mb-0 px-6 py-3 rounded-full text-white font-semibold transition-colors duration-300 ${
              isListeningRef.current ? "bg-red-500 hover:bg-red-600" : "bg-green-500 hover:bg-green-600"
            }`}
          ...
          {isListening && (
            <div className="flex items-center">
              <div className={`w-4 h-4 rounded-full mr-2 ${isSpeaking ? "bg-green-500" : "bg-gray-300"}`}></div>
      ...
      <div className="mb-8 bg-white rounded-lg shadow-md p-6">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <ToggleSwitch id="beep-toggle" label="Geluidssignaal" checked={playBeep} onChange={setPlayBeep} />
          <ToggleSwitch
            id="stats-toggle"
      ...
      <div className="mb-8 flex items-center justify-center">
        ...
        >
          {isScreenActiveEnabled ? <Lock className="w-4 h-4 mr-2" /> : <LockOpen className="w-4 h-4 mr-2" />}
